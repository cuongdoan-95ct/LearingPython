...
Các API ứng dụng web (Web API) là nền tảng thiết yếu của thế giới kết nối hiện nay,  
giúp phần mềm giao tiếp với nhau, từ ứng dụng điện thoại đến các máy chủ backend ẩn sâu.  
Dù hệ thống dựa trên API có quy mô hay mục đích thế nào, việc thiết kế API luôn phải là mối quan tâm chính,  
vì sự thành công hay thất bại của hệ thống phụ thuộc trực tiếp vào chất lượng thiết kế của tất cả các API của nó.  
