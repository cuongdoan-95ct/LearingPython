...
*   **Phân tích một lệnh gọi REST API**:
    ...
    *   Yêu cầu này bao gồm **phương thức HTTP** là `GET` và **đường dẫn (path)** là `/products/P123`.
        *   Đường dẫn là một địa chỉ xác định một **tài nguyên (resource)** trên máy chủ; trong trường hợp này, nó xác định sản phẩm P123 trong tập hợp các sản phẩm (`products`).
        *   Phương thức HTTP (`GET`) cho biết người dùng muốn làm gì với tài nguyên này: `GET` có nghĩa là họ muốn **truy xuất (retrieve)** tài nguyên.
...
*   **Các nguyên tắc cơ bản của REST API**:
    ...
    * **Tài nguyên (Resources)**: Mọi thứ trong API REST đều được biểu diễn dưới dạng tài nguyên, chẳng hạn như "User", "Post", hoặc "Comments". Các tài nguyên được xác định bằng URI (Uniform Resource Identifier), ví dụ: `/users/123`.
    * **Phương thức HTTP**: REST API sử dụng giao thức HTTP để cho phép phần mềm tương tác với phần mềm. Sử dụng các phương thức HTTP tiêu chuẩn để thực hiện các hành động trên tài nguyên (GET để lấy, POST để tạo, v.v.). Các phương thức HTTP như `GET`, `POST`, `PUT`, `PATCH`, và `DELETE` được sử dụng để ánh xạ tới các hành động tiêu chuẩn trên tài nguyên (thường được gọi là các chức năng CRUD: Create, Read, Update, Delete - Tạo, Đọc, Cập nhật, Xóa).
...
Quá trình thiết kế giao diện lập trình REST API là việc chuyển đổi các mục tiêu API (đã xác định bằng API goals canvas) và các đầu vào, đầu ra của chúng thành các cấu trúc của REST API:
